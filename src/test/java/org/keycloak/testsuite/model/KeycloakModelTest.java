...
import org.keycloak.testsetup.Config;
import org.keycloak.testsetup.KeycloakModelParameters;
...
public abstract class KeycloakModelTest {
    ...
    /**
     * Convenience method for {@link #inCommittedTransaction(java.util.function.Consumer)} that obtains
     * realm model from the session and puts it into session context before running the {@code fn}
     * task.
     */
    protected void withRealm(
            String realmId,
            BiConsumer<KeycloakSession, RealmModel> fn
    ) {
        inCommittedTransaction(
                session -> {
