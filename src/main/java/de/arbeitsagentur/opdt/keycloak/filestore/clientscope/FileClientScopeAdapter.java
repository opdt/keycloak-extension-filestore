...
import de.arbeitsagentur.opdt.keycloak.filestore.client.FileProtocolMapperUtils;
import java.util.*;
...
public class FileClientScopeAdapter extends AbstractClientScopeModel<FileClientScopeEntity>
  ...
  public FileClientScopeAdapter(
      KeycloakSession session, RealmModel realm, FileClientScopeEntity entity) {
            ...
            Collectors.toMap(
                ...
                entry -> {
                  if (entry.getValue().isEmpty()) {
                    ...
                    LOG.warnf(
                        "ClientScope '%s' realm '%s' has attribute '%s' with %d values, retrieving only the first",
                        getName(), getRealm().getName(), entry.getKey(), entry.getValue().size());
                  }
