...
import static org.keycloak.models.map.common.AbstractFileProviderFactory.MapProviderObjectType.ROLE_AFTER_REMOVE;
import static org.keycloak.models.map.common.AbstractFileProviderFactory.MapProviderObjectType.ROLE_BEFORE_REMOVE;
...
public class FileRoleProvider implements RoleProvider {
    ...
    public Stream<RoleModel> getRealmRolesStream(RealmModel realm, Integer first, Integer max) {
        Stream<RoleModel> rolesStream = FileRoleStore.readAll().stream()
    ...
    public Stream<RoleModel> getRolesStream(RealmModel realm, Stream<String> ids, String search, Integer first, Integer max) {
        ...
        Stream<RoleModel> roleStream = ids
                .filter(id -> FileRoleStore.exists(id, realm.getId()))
                .map(id -> FileRoleStore.read(id, realm.getId()))
                .map(entityToAdapterFunc(realm));
        if (search != null) {
            return roleStream.filter(entity -> SearchPatterns.insensitiveLike(entity.getName(), "%" + search + "%"));
        }
        return roleStream;
    }
    ...
    public Stream<RoleModel> searchForRolesStream(RealmModel realm, String search, Integer first, Integer max) {
        ...
        if (!search.isBlank()) {
            roleStream = roleStream.filter(entity ->
