...
import org.keycloak.models.map.storage.file.SearchPatterns;
...
public abstract class FileAdminEventQuery implements AdminEventQuery {
    ...
    public Stream<AdminEvent> getResultStream() {
        Comparator<AdminEvent> ENTITY_COMPARATOR = Comparator
                .comparing(AdminEvent::getTime, "DESC".equals(this.order) ? Comparator.reverseOrder() : Comparator.naturalOrder());
        Stream<AdminEvent> adminEvents = read()
                .filter(ev -> this.realmId == null || this.realmId.equals(ev.getRealmId()))
                .filter(ev -> this.authRealmId == null || (ev.getAuthDetails() != null && this.authRealmId.equals(ev.getAuthDetails().getRealmId())))
                .filter(ev -> this.authClientId == null || (ev.getAuthDetails() != null && this.authClientId.equals(ev.getAuthDetails().getClientId())))
                .filter(ev -> this.authUserId == null || (ev.getAuthDetails() != null && this.authUserId.equals(ev.getAuthDetails().getUserId())))
                .filter(ev -> this.authIpAddress == null || (ev.getAuthDetails() != null && this.authIpAddress.equals(ev.getAuthDetails().getIpAddress())))
                .filter(ev -> this.operationTypes == null || ev.getOperationType() != null && this.operationTypes.contains(ev.getOperationType()))
                .filter(ev -> this.resourceTypes == null || ev.getResourceType() != null && this.resourceTypes.contains(ev.getResourceType()))
                .filter(ev -> this.resourcePath == null || (ev.getResourceType() != null && SearchPatterns.like(ev.getResourcePath(), this.resourcePath.replace('*', '%'))))
