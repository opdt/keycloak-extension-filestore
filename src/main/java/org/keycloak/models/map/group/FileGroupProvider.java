...
import static org.keycloak.models.map.common.AbstractFileProviderFactory.MapProviderObjectType.GROUP_AFTER_REMOVE;
import static org.keycloak.models.map.common.AbstractFileProviderFactory.MapProviderObjectType.GROUP_BEFORE_REMOVE;
...
public class FileGroupProvider implements GroupProvider {
    ...
    public Stream<GroupModel> getGroupsStream(RealmModel realm, Stream<String> ids, String search, Integer first, Integer max) {
        var groups = ids
                .filter(groupId -> FileGroupStore.exists(groupId, realm.getId()))
    ...
    public Stream<GroupModel> getTopLevelGroupsStream(RealmModel realm, String search, Boolean exact, Integer firstResult, Integer maxResults) {
        LOG.tracef("getTopLevelGroupsStream(%s, %s, %b, %s, %s)%s", realm, search, exact, firstResult, maxResults, getShortStackTrace());
        Stream<GroupModel> groups = FileGroupStore.readAll().stream()
                .filter(group -> realm.getId().equals(group.getRealmId()))
    ...
    public Stream<GroupModel> searchForGroupByNameStream(RealmModel realm, String search, Boolean exact, Integer firstResult, Integer maxResults) {
        ...
        return paginatedStream(groups, firstResult, maxResults)
                ...
                //todo: this mapping makes no sense at all because we are filtering by group models that have no parent beforehand -> check it again
    ...
    public Stream<GroupModel> searchGroupsByAttributes(RealmModel realm, Map<String, String> attributes, Integer firstResult, Integer maxResults) {
        Stream<FileGroupEntity> groups = FileGroupStore.readAll().stream()
                .filter(group -> realm.getId().equals(group.getRealmId()));
    ...
    public GroupModel createGroup(RealmModel realm, String id, String name, GroupModel toParent) {
        ...
        FileGroupStore.readAll().stream()
                ...
                .filter(group -> toParent == null ? group.getParentId() == null : toParent.getId().equals(group.getParentId()))
                .findAny()
                .ifPresent(group -> {
    ...
    public void moveGroup(RealmModel realm, GroupModel group, GroupModel toParent) {
        ...
        FileGroupStore.readAll().stream()
                ...
                .filter(groupEntity -> toParent == null ? groupEntity.getParentId() == null : toParent.getId().equals(groupEntity.getParentId()))
                .findAny()
                .ifPresent(groupEntity -> {
                    throw new ModelDuplicateException("Group with name '" + group.getName() + "' in realm " + realm.getName() + " already exists for requested parent");
                });
